...
class KafkaEventConsumerTest extends AnyFunSuite with MockFactory {
    ...
    val schema=new StructType().
        //add("ids",IntegerType).
        add("ids",StringType).
        add("error",StringType).add("source",StringType).add("code",StringType).add("solution",StringType).add("CodeWithNoComments",StringType).add("SolutionWithNoComments",StringType).add("user",StringType).add("group",StringType)
    ...
    test("kafka Consumer return a dataframe with no null values from json(test for schema control)") {
        val mockSparkFacade = mock[SparkFacade]
        import spark.implicits._
