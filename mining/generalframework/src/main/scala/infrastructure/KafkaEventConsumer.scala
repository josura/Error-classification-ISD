...
class KafkaEventConsumer(val _spark:SparkFacade) extends EventConsumer(_spark){
  override def Consume(topics:String,schema:StructType): Dataset[Row] = {
      ...
      val UserCode= CodeCleaner.cleanCode(StringDF.select(from_json(col("value"),schema).as("data")).select("data.*"),"code","cleanedCode",streaming = true)  //problems if I do not send json data
      UserCode
  }
}
